...
struct action{
    ...
    struct lazy{
        int op, v;
        lazy(int op = 0, int v = 0) : op(op), v(v){}
    };
    static node op(const node& l, const node& r){
        return node{};
    }
    static bool is_lz_id(const lazy& now){ return now.op == 0; }
    ...
    static void compose(lazy& s, const lazy& p){ s = p; }
};
