...
void run(){
    cin >> n >> m; seg = {MAX}; sum = {MAX};
    for(int i = 1;i <= n;i++){
        cin >> arr[i];
        seg.set(arr[i], 1); sum.set(arr[i], arr[i]);
    }
    ...
    while(m--){
        ...
        if(op == 1){
            ...
            if(a >= b) cout << a * seg.query(0, MAX - 1).v << "\n";
            else{
                ll now = 0;
                now += a * seg.query(0, a).v;
                now += b * seg.query(b, MAX - 1).v;
                now += sum.query(a + 1, b - 1).v;
                cout << now << "\n";
            }
        }
    }
