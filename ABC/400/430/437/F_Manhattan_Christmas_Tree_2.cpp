...
struct monoid{
    ...
    static num get_max(const num& l, const num& r){
        num ret;
        ret.a = max(l.a, r.a);
        ret.b = max(l.b, r.b);
        ret.c = max(l.c, r.c);
...
void run(){
    cin >> n >> m; seg = {n};
    for(int i = 1;i <= n;i++) cin >> a[i].x >> a[i].y, seg.set(i, a[i]);
    while(m--){
        ll op; cin >> op;
        if(op == 1){
            ...
            ll now = 0;
            now = max(now, q.mx.a - (x + y));
            now = max(now, q.mx.b - (x - y));
            now = max(now, q.mx.c - (-x + y));
            now = max(now, q.mx.d - (-x - y));
