...
ll num(char c, bool in){
    return n * m + (in ? 26 : 0) + c - 'a';
}
    ...
    for(int i = 0;i < n;i++){
        for(int j = 0;j < m;j++){
            ...
            for(int k = 0;k < 4;k++){
                ...
                dij.addsol(idx(i, j), idx(ny, nx), 1);
            }

            if(c >= 'a' && c <= 'z'){
                dij.addsol(idx(i, j), num(c, 1), 1);
                dij.addsol(num(c, 0), idx(i, j), 0);
            }
        }
    }

    for(int i = 'a';i <= 'z';i++) dij.addsol(num(i, 1), num(i, 0), 0);

    dij.init(0);
    ll ret = dij.ret(idx(n - 1, m - 1));
    cout << (ret >= INF ? -1 : ret);
}
