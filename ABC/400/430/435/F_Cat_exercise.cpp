...
ll dnc(int l, int r){
    if(l >= r) return 0;
    ll mi = seg.query(l, r).idx;
    ll li = seg.query(l, mi - 1).idx;
    ll ri = seg.query(mi + 1, r).idx;

    ll ret = 0;
    ll lr = dnc(l, mi - 1), rr = dnc(mi + 1, r);
    ret = max(ret, lr + (li ? abs(mi - li) : 0));
    ret = max(ret, rr + (ri ? abs(mi - ri) : 0));
    return ret;
}

void run(){
    cin >> n; seg = {n};
    for(int i = 1;i <= n;i++) cin >> a[i], seg.set(i, {a[i], i});
    cout << dnc(1, n);
}
