#include <bits/stdc++.h>
#define fastio cin.tie(0), cout.tie(0), ios::sync_with_stdio(0);
#define all(x) (x).begin(), (x).end()
#define x first 
#define y second
using namespace std; using ll = long long; using u64 = uint64_t;
using ld = long double; using pld = pair<ld, ld>;
using i128 = __int128_t; using f128 = __float128; 
using pll = pair<ll, ll>; using tll = tuple<ll, ll, ll>;
ll n, m, k, t = 1; string s;

constexpr ll INF = 0x3f3f3f3f3f3f3f3f;
constexpr ll MINF = 0xc0c0c0c0c0c0c0c0;
constexpr ll MAX = 201010; // SET MAX SIZE
constexpr ll MOD = 998244353;

struct uf_policy{
    struct node{
        int sz;
        node(int sz = 0) : sz(sz){} // identity
    };
    static void merge(node& a, node& b){
        a.sz += b.sz;
    }
};

template <class policy = uf_policy>
class _uf{ // 1-based index
private:
    using node = typename policy::node;
    vector<int> p, sz; int n, cc;
    node id() const{ return node(); }
public:
    vector<node> arr;   
    _uf(int n = 0){ clear(n); } // O(n)
    void clear(int n){ // O(n)
        this->n = n; cc = n;
        p.assign(n + 1, -1); sz.assign(n + 1, 1);
        arr.assign(n + 1, id());
    }

    int find(int x){ // O(1)
        if(p[x] == -1) return x;
        return p[x] = find(p[x]);
    }

    bool merge(int a, int b){ // O(1) + policy::merge
        a = find(a); b = find(b);
        if(a == b) return false;
        if(sz[a] < sz[b]) swap(a, b);
        p[b] = a; sz[a] += sz[b]; cc--;
        policy::merge(arr[a], arr[b]); arr[b] = id();
        return true;
    }

    int count() const{ return cc; } // O(1)
    node& get(int x){ return arr[find(x)]; } // O(1)
    void set(int x, const node& v){ arr[find(x)] = v; } // O(1)
    int size(int x){ return sz[find(x)]; } // O(1)
    bool root(int x){ return find(x) == x; } // O(1)
    bool same(int a, int b){ return find(a) == find(b); } // O(1)
}; _uf uf;

class _bs { // integer
public:
    template <class F>
    static ll max_right(ll l, ll r, const F& f) {
        ll lo = l, hi = r, mid = -1;
        while (hi > lo) {
            mid = (hi + lo + 1) >> 1ll;
            if (f(mid)) lo = mid;
            else hi = mid - 1;
        }
        return lo;
    }

    template <class F>
    static ll min_left(ll l, ll r, const F& f) {
        ll lo = l, hi = r, mid = -1;
        while (hi > lo) {
            mid = (hi + lo) >> 1ll;
            if (f(mid)) hi = mid;
            else lo = mid + 1;
        }
        return lo;
    }
};

vector <pair<int, int>> query;
vector <int> adj[MAX];
char v[MAX]; bool flag;

void dfs(int cur, int num){
    if(v[cur] != -1){
        if(v[cur] != num) flag = 0;
        return;
    }

    v[cur] = num;
    for(auto& nxt : adj[cur]) dfs(nxt, num ^ 1);
}

bool f(int cur){
    for(int i = 1;i <= n;i++) adj[i].clear();
    memset(v, -1, sizeof(v));
    for(int i = 0;i <= cur;i++){
        auto& [s, e] = query[i];
        adj[s].push_back(e);
        adj[e].push_back(s);
    }

    flag = 1;
    for(int i = 1;i <= n;i++){
        if(v[i] != -1) continue;
        dfs(i, 0);
    }

    return !flag;
}

int result;
void run(){
    cin >> n >> m; uf.clear(2 * n);
    for(int i = 1;i <= m;i++){
        int s, e; cin >> s >> e;
        query.push_back({s, e});
    }

    for(int i = 1;i <= n;i++) uf.set(i, 1);
    int idx = _bs::min_left(0, m, f);
    for(int i = 0;i < m;i++){
        if(i >= idx){
            cout << -1 << "\n";
            continue;
        }

        auto& [s, e] = query[i];
        if(!uf.same(s, e + n)){
            result -= min(uf.get(s).sz, uf.get(s + n).sz);
            result -= min(uf.get(e).sz, uf.get(e + n).sz);

            uf.merge(s, e + n);
            uf.merge(e, s + n);
            result += min(uf.get(s).sz, uf.get(s + n).sz);
        }
        
        cout << result << "\n";
    }
}

int main() {
    fastio; // cin >> t;
    while(t--) run(); 

    return 0;
}
