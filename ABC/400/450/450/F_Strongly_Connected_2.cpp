...
struct action{
    ...
    static bool is_lz_id(const lazy& now){ return now.v == 1; }
    static void apply(node& seg, const lazy& lz){ seg.v = (seg.v * lz.v) % MOD; }
    static void compose(lazy& s, const lazy& p){ s.v = (s.v * p.v) % MOD; }
};
...
void run(){
    ...
    for(int i = 2;i <= n;i++){
        for(auto& j : adj[i]){
            ll now = seg.query(j, i - 1).v;
            seg.update(1, j - 1, 2);
            seg.update(i, 2);

            now += seg.query(i).v; now %= MOD;
            seg.set(i, now);
        }
    }
