...
struct action{
    struct node {
        ll v, mx, sz;
        node(ll v = 0, ll mx = 0, ll sz = 0) : v(v), mx(mx), sz(sz){}
    };
    struct lazy{
        ll v; bool f, c;
        lazy(ll v = 0, bool f = 0, bool c = 0) : v(v), f(f), c(c){}
    };
    ...
    static bool is_lz_id(const lazy& now){ 
        return now.v == 0 && now.f == 0 && now.c == 0; 
    }
    static void apply(node& seg, const lazy& lz){
        if(lz.c) seg.mx = 0;
        if(lz.f) seg.v = seg.sz - seg.v, seg.mx = 0;
        if(seg.v) seg.mx += lz.v;
    }
...
void run(){
    cin >> n >> m;
    for(int i = 1;i <= n;i++) tmp[i].sz = tmp[i].v = 1;
    seg.build(tmp);
    ...
    while(m--){
        ...
        if(op == 1){
            ...
            seg.update(l, r, {0, 1, 1});
        }
        else{
            auto q = seg.query(l, r).mx;
            cout << q << "\n";
        }
    }
}
